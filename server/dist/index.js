...
if (fs.existsSync(distDir)) {
    ...
    console.warn("[tradeverify] dist/ not found — only /api/* routes work. From repo root run `npm run build`, or in dev use Vite (e.g. http://localhost:5173) with the API on PORT.");
    app.get("/", (_req, res) => {
        res
            .status(503)
            .type("text")
...
if (port === 443 && !useTls) {
    console.warn("[tradeverify] PORT is 443 but SSL_KEY_FILE + SSL_CERT_FILE are missing or files not found — starting plain HTTP (browsers expect HTTPS on 443).");
}
const onListen = () => {
    ...
    void (async () => {
        try {
            ...
            if (n > 0) {
                console.log(`[tradeverify] removed ${n} member profile(s) expired past 30-day grace`);
            }
        }
        catch (e) {
