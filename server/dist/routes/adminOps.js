...
import { invalidateSmtpTransportCache, notifyApplicationDecision, notifyApplicantApprovedForPayment, notifyApplicantVerificationOutcome, } from "../lib/adminMail.js";
...
router.get("/dashboard", async (_req, res) => {
    try {
        ...
        const membersPortal = settledValue(membersPortalResult, "membersPortal", 0);
        const guidesTotal = settledValue(guidesTotalResult, "guidesTotal", 0);
        const applicationsPending = settledValue(applicationsPendingResult, "applicationsPending", 0);
        const settings = settledValue(settingsResult, "organizationSettings", {
            revenueMtdCents: 0,
            outstandingCents: 0,
        });
        const recentApps = settledValue(recentAppsResult, "recentApps", []);
        const recentMembers = settledValue(recentMembersResult, "recentMembers", []);
        const recentGuides = settledValue(recentGuidesResult, "recentGuides", []);
        const activity = [
            ...recentApps.map((a) => ({
...
async function patchOrganizationSettings(req, res) {
    try {
        ...
        const billingEnabled = typeof body.billingEnabled === "boolean"
            ? body.billingEnabled
            : undefined;
        const goCardlessPublishableKey = typeof body.goCardlessPublishableKey === "string"
        ...
        const goCardlessSecretKey = typeof body.goCardlessSecretKey === "string"
            ...
            : undefined;
        const goCardlessWebhookSecret = typeof body.goCardlessWebhookSecret === "string"
    ...
    return {
        ...
        createdMember: createdMember
            ? {
                ...
                goCardlessSubscriptionStatus: createdMember.goCardlessSubscriptionStatus ?? null,
            }
            : null,
...
async function ensureSumsubApplicantForApplication(id) {
    ...
    if (application.verificationProvider === "sumsub" &&
...
router.post("/applications/:id/sumsub-link", async (req, res) => {
    try {
        ...
        if (!ensured.application.registrationFeePaidAt) {
            res.status(400).json({
                error: "Record the registration fee first. Verification now starts after the upfront application payment.",
            });
            return;
        }
...
router.post("/applications/:id/provision-member", async (req, res) => {
    try {
        ...
        if (prov.kind === "membership_expiry_missing") {
            res.status(400).json({
                error: "Membership payment is recorded without an expiry date. Re-save the membership payment before creating the profile.",
            });
            return;
        }
...
router.post("/applications/:id/record-manual-payment", async (req, res) => {
    try {
        ...
        if (type === "registration_fee") {
            ...
            if (!before.membershipSubscribed) {
                ...
                if (before.manualMembershipExpiresAt == null) {
                    res.status(400).json({
                        error: "This membership was recorded via GoCardless. Manage renewal in GoCardless or have the member subscribe from the portal.",
                    });
                    return;
                }
                ...
                if (memberRow?.membershipBillingType === "goCardless") {
                    res.status(400).json({
                        error: "This member is on GoCardless billing; dates sync from the subscription.",
                    });
                    return;
                }
                await prisma.application.update({
        ...
        if (prov.kind === "membership_expiry_missing") {
            res.status(400).json({
                error: "Membership payment is recorded without an expiry date. Enter an expiry date and try again.",
            });
            return;
        }
        if (prov.kind === "not_approved") {
