...
router.post("/login", async (req, res) => {
    try {
        ...
        if (staff.totpEnabled && staff.totpSecret) {
            const pendingToken = jwt.sign({ sub: staff.id, k: "2fa" }, jwtSecret(), { expiresIn: "5m" });
            res.json({
                requires2fa: true,
                pendingToken,
                staff: {
        ...
        const token = jwt.sign({ sub: staff.id, email: staff.email, role: "staff" }, jwtSecret(), { expiresIn: "7d" });
        res.json({
            token,
            staff: {
                id: staff.id,
