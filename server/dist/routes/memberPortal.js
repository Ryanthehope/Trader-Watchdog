...
function memberDocDir(memberId) {
    return path.join(UPLOAD_ROOT, memberId);
}
function ensureMemberDocDir(memberId) {
...
router.post("/profile-logo", requireMemberMembershipActive, (req, res, next) => {
    profileLogoUpload.single("logo")(req, res, (err) => {
        if (err) {
            res.status(400).json({
    ...
    try {
        ...
        if (existing?.profileLogoStoredName) {
            try {
                fs.unlink(memberProfileLogoFilePath(memberId, existing.profileLogoStoredName), () => { });
            }
            catch {
                /* ignore */
            }
...
router.put("/profile", requireMemberMembershipActive, async (req, res) => {
    try {
        const memberId = req.memberId;
        const { name, trade, location, blurb, documentAccentHex, invoiceAddress, invoiceBankDetails, invoicePhone, invoiceEmail, vatNumber, vatRegistered, documentLayout, } = req.body ?? {};
        if (name === undefined ||
            trade === undefined ||
            location === undefined ||
            blurb === undefined) {
        ...
        const hexRaw = documentAccentHex !== undefined
        ...
        if (hexRaw !== undefined &&
            ...
            res.status(400).json({
                error: "Accent colour must be a hex value like #0d9488 (6 digits after #)",
            });
            return;
        }
        const m = await prisma.member.update({
            ...
            data: {
                ...
                ...(documentAccentHex !== undefined
                    ? {
                        documentAccentHex: hexRaw === "" ? null : hexRaw.toLowerCase(),
                    }
                    : {}),
                ...(invoiceAddress !== undefined
                    ? {
                        invoiceAddress: String(invoiceAddress ?? "").trim() || null,
                    }
                    : {}),
                ...(invoiceBankDetails !== undefined
                    ? {
                        invoiceBankDetails: String(invoiceBankDetails ?? "").trim() || null,
                    }
                    : {}),
...
router.post("/documents", requireMemberMembershipActive, (req, res, next) => {
    ...
    try {
        ...
        if (existingDocumentCount >= MAX_MEMBER_DOCUMENTS) {
            fs.unlink(file.path, () => { });
            res.status(400).json({
                error: "You can keep up to 5 insurance and qualification documents on your account. Delete one before uploading another.",
            });
            return;
        }
...
router.get("/documents/:id/file", requireMemberMembershipActive, async (req, res) => {
    try {
        const memberId = req.memberId;
        ...
        res.setHeader("Content-Disposition", `attachment; filename="${encodeURIComponent(doc.originalName).replace(/'/g, "%27")}"`);
