...
router.post("/applications", (req, res, next) => {
    ...
    try {
        ...
        const identifiablePersonAddress = String(req.body?.identifiablePersonAddress ?? "").trim();
        const email = String(req.body?.email ?? "").trim().toLowerCase();
        const phone = String(req.body?.phone ?? "").trim();
        const postcode = String(req.body?.postcode ?? "").trim();
        const wasteCarrierRequired = String(req.body?.wasteCarrierRequired ?? "").trim();
        ...
        const hasExtendedFields = Boolean(legalStructure ||
            tradingAddress ||
            identifiablePerson ||
        ...
        if (hasExtendedFields &&
            (!legalStructure ||
                ...
                !gasSafeRequired)) {
            res.status(400).json({
                error: "Please complete the business structure, address, identifiable person, and licence requirement fields.",
            });
            return;
        }
        if (hasExtendedFields &&
