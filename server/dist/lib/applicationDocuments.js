...
const UPLOAD_ROOT = process.env.APPLICATION_UPLOAD_DIR?.trim() ||
...
export function applicationDocDir(applicationId) {
    return path.join(UPLOAD_ROOT, applicationId);
}
/** Resolved absolute path to a stored file, or null if outside the app folder. */
export function applicationDocumentResolvedPath(applicationId, storedName) {
...
export function ensureApplicationDocDir(applicationId) {
    const dir = applicationDocDir(applicationId);
    fs.mkdirSync(dir, { recursive: true });
    return dir;
}
export async function removeApplicationUploadDir(applicationId) {
    const dir = applicationDocDir(applicationId);
    try {
...
export async function persistApplicationDocuments(applicationId, files) {
    if (!files.length)
        return;
