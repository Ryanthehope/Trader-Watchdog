...
export const JOB_TRADE_CATEGORIES = [
...
export function isValidJobTradeSlug(slug) {
    return SLUGS.has(slug.trim().toLowerCase());
}
export function jobTradeLabelForSlug(slug) {
 ...
 */
export function memberTradeMatchesCategory(memberTrade, slug) {
    ...
    switch (s) {
        case "it":
            return (/\b(it|i\.t\.|ict|tech|computer|computing|network|software|cyber|digital|web|developer|programming|data)\b/i.test(memberTrade) ||
        ...
        case "plumbing":
            return (t.includes("plumb") ||
                t.includes("heating") ||
                t.includes("gas safe") ||
                t.includes("boiler") ||
                t.includes("bathroom fit"));
        case "building":
            return (t.includes("builder") ||
                t.includes("building") ||
                t.includes("construction") ||
                t.includes("brick") ||
                t.includes("extension"));
        case "carpentry":
            return (t.includes("carpent") || t.includes("joiner") || t.includes("wood"));
        case "roofing":
            return t.includes("roof");
        case "decorating":
            return (t.includes("decor") || t.includes("paint") || t.includes("plaster"));
        case "landscaping":
            return (t.includes("landscape") ||
                t.includes("garden") ||
                t.includes("grounds") ||
                t.includes("tree") ||
                t.includes("lawn"));
        case "security":
            return (t.includes("security") ||
                t.includes("alarm") ||
                t.includes("cctv") ||
...
export async function findMembersMatchingJobTrade(prisma, tradeSlug) {
    ...
    if (slug === "other") {
        const broad = rows.filter((m) => isMemberPublicListingVisible({
            membershipUnlimited: m.membershipUnlimited,
            membershipBillingType: m.membershipBillingType,
            membershipExpiresAt: m.membershipExpiresAt,
            stripeSubscriptionStatus: m.stripeSubscriptionStatus,
        ...
        if (out.length > 0)
            return out.map((m) => ({ id: m.id, trade: m.trade }));
