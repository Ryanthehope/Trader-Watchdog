 ...
 */
export async function fetchGa4OverviewReport(org) {
    ...
    if (!client && dbJson) {
        try {
            const credentials = JSON.parse(dbJson);
    ...
    if (!propertyId || !client) {
        return {
            configured: false,
            metrics: null,
            hint: "To show Google stats here: in Integrations → Google Analytics, set the numeric Property ID and paste your service account JSON (or use GA4_PROPERTY_ID + GOOGLE_APPLICATION_CREDENTIALS in server .env). Add the service account email as Viewer in GA4 → Admin → Property access management.",
            error: null,
        };
    }
    try {
        ...
        if (!vals || vals.length < 3) {
            return {
                ...
                hint: "GA returned no rows yet — traffic may still be processing, or the site has no hits in this range.",
                error: null,
            };
        }
        const num = (i) => Number.parseInt(String(vals[i]?.value ?? "0"), 10) || 0;
        return {
            configured: true,
            metrics: {
                activeUsers: num(0),
        ...
        return {
            configured: true,
            metrics: null,
            hint: "Ensure the service account email is added under GA4 → Admin → Property access management (Viewer).",
            error: msg,
        };
    }
}
