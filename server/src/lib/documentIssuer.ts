...
export function documentIssuerFromMember(m: Member) {
  const email =
    m.invoiceEmail?.trim() || m.loginEmail?.trim() || null;
...
export type DocumentIssuerPublic = ReturnType<typeof documentIssuerFromMember>;
