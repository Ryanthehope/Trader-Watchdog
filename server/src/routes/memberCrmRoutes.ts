...
function jsonLineItems(raw: unknown): { ok: true; items: unknown[] } | { ok: false } {
...
function lineItemsSummaryJson(lineItems: unknown): string {
  ...
  for (const row of lineItems) {
    ...
    if (d || gbp) parts.push(d ? `${d} (${gbp} GBP)` : gbp);
  }
  return parts.join("; ");
}
...
router.post("/quotes", requireMemberMembershipActive, async (req, res) => {
  try {
    ...
    if (!customerName || !customerAddress || !title || !parsed.ok) {
      res.status(400).json({
        error:
          "customerName, customerAddress, title, and lineItems (array) are required",
      });
      return;
    }
    let subtotal = 0;
    for (const row of parsed.items) {
      const r = row as { lineTotalPence?: unknown };
      const n = Number(r?.lineTotalPence);
      if (Number.isFinite(n)) subtotal += Math.round(n);
    }
    const vatPence = Math.round(
      Number(req.body?.vatPence ?? 0) || 0
    );
    ...
    const row = await prisma.memberQuote.create({
      data: {
        ...
        title,
        lineItems: parsed.items as Prisma.InputJsonValue,
    ...
    const row = await prisma.memberQuote.update({
      ...
      data: {
        ...
        ...(req.body?.customerEmail !== undefined
          ? {
              customerEmail:
                String(req.body.customerEmail ?? "").trim() || null,
            }
          : {}),
...
async function deleteMemberQuote(
  req: Request,
  res: Response,
  memberId: string
): Promise<void> {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="customer-invoices-export.csv"'
      );
      res.send(body);
    } catch (e) {
      console.error(e);
  ...
  async (req, res) => {
    try {
      ...
      if (!customerName || !customerAddress || !parsed.ok) {
        res.status(400).json({
          error:
            "customerName, customerAddress, and lineItems (array) are required",
        });
        return;
      }
      let subtotal = 0;
  ...
  async (req, res) => {
    try {
      ...
      if (existing.status === "paid") {
        res.status(400).json({
          error:
            "This invoice is marked paid. You can add notes via the quick edit, but amounts and line items cannot be changed.",
        });
        return;
      }
      const customerName = String(req.body?.customerName ?? "").trim();
