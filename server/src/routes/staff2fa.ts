...
function prisma2faSetupError(
  ...
  if (e instanceof Prisma.PrismaClientKnownRequestError) {
    if (e.code === "P2022") {
      return {
        status: 503,
        error:
          "Database is missing 2FA columns on Staff (totpSecret, totpEnabled). On the server run: npx prisma db push — or apply migrations — then restart.",
      };
    }
    if (e.code === "P2021") {
      return {
        status: 503,
        error:
          "Database table missing (Prisma P2021). Run npx prisma db push on the server and restart Node.",
      };
    }
    if (e.code === "P2025") {
  ...
  if (
    /no such column|unknown column|does not exist|SQLITE_ERROR/i.test(msg)
  ) {
    return {
      status: 503,
      error:
        "Database schema looks out of date (2FA fields missing). On the server: cd to the server app folder, run npx prisma db push, then restart Node.",
    };
  }
  return null;
}

export function registerStaff2faRoutes(router: Router): void {
